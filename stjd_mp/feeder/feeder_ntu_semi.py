...
class Feeder(Dataset):
    def __init__(self, data_path, label_path=None, p_interval=1, split='train', data_ratio=None,
                 ...
                 window_size=-1, normalization=False, debug=False, use_mmap=True, bone=False, vel=False , seeds =0 ):
        """
        data_path:
        label_path:
    ...
    def random_select_data(self, data_ratio):
        ...
        shuffle_seed = int(time.time() * 1000) % 10000  # Generate a random seed for shuffling
        np.random.seed(shuffle_seed)  # Set the new seed
        np.random.shuffle(idx)  # Shuffle with the new seed
