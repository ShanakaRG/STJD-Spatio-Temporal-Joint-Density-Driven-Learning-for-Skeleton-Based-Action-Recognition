...
def evaluate(data_loader, model, device):
    ...
    for batch in metric_logger.log_every(data_loader, 10, header):
        ...
        # print(torch.max(output.data, 1).shape)
        # exit()
        # print("8888==")
        # print(target)
    ...
    # Open a CSV file to write the labels and predictions
    with open('predictions_model2.csv', mode='w', newline='') as file:
        writer = csv.writer(file)
        
        # Write header row
        writer.writerow(['true_label', 'pred_label'])
        
        # Write each pair of true label and prediction
