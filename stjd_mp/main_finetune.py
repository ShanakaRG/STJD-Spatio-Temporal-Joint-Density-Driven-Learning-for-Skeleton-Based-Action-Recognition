...
def save_arg(arg):
    ...
    with open('{}/config.yaml'.format(arg.output_dir), 'w') as f:
...
def main(args):
    ...
    # #to check all learnable layers
    # for name, param in model.named_parameters():
    #     if param.requires_grad:
    #         print(name)

    # # to verify if label diff mlp is correctly loaded
    # # Define the specific keys to verify
    # keys_to_verify = ['lable_diff_decoder.mlp_head.0.weight', 'lable_diff_decoder.mlp_head.0.bias']

    # # Loop through each key and compare
    # for key in keys_to_verify:
    #     if key in checkpoint_model:
    #         # Extract weights from model and checkpoint
    #         model_weights = model.state_dict().get(key)
    #         checkpoint_weights = checkpoint_model[key]

    #         # Check if the weights match
    #         if model_weights is not None and torch.equal(model_weights, checkpoint_weights):
    #             print(f"Weights for {key} are correctly loaded.")
    #         else:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # test_stats,each_acc,confusion  = evaluate(data_loader_val, model, device)
        # print(f"Accuracy of the network on the {len(dataset_val)} test images: {test_stats['acc1']:.1f}%")
        ...
        test_stats,each_acc,confusion,top1,top5 = evaluate(data_loader_val, model, device)
        print(f"Accuracy of the network on the {len(dataset_val)} test images: {test_stats['acc1']:.1f}%")
        max_accuracy = max(max_accuracy, test_stats["acc1"])
        max_accuracy2 = max(max_accuracy2, 100*top1)
        print(f'Max accuracy: {max_accuracy:.2f}%')
        print(f'Max accuracy-2: {max_accuracy2:.2f}%')
        if max_accuracy == test_stats["acc1"]: 
            max_epaoch = epoch
        if max_accuracy2 == top1: 
            max_epaoch2 = epoch
        ...
        if max_accuracy == test_stats["acc1"]: 
            ...
            if args.output_dir:
                misc.save_model(
                    args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
                    loss_scaler=loss_scaler, epoch=epoch,name=1)
        ...
        if max_accuracy2 == top1: 
            ...
            if args.output_dir:
                misc.save_model(
                    args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
                    loss_scaler=loss_scaler, epoch=epoch,name=2)

        with open('{}/epoch{}_test_each_class_acc.csv'.format(args.log_dir, epoch + 1), 'w') as f:
            writer = csv.writer(f)
        ...
        log_stats = {**{f'train_{k}': v for k, v in train_stats.items()},
                        **{f'test_{k}': v for k, v in test_stats.items()},
                        ...
                        'Max accuracy 2': max_accuracy2,
                        'max acc epaoch': max_epaoch,
                        'max acc epaoch2': max_epaoch2 }
