...
class Gen_mask(nn.Module):
        def __init__(self,no_of_channels=3,output_channels=64, no_of_joints=25,kernel_size = 3) :
            super (Gen_mask, self).__init__()
            self.weights =  nn.Parameter(torch.ones(output_channels))
            self.h_values =  nn.Parameter(torch.ones(no_of_joints,output_channels))
            self.joint_weights = nn.Parameter(torch.ones(no_of_joints))
            self.m = nn.Conv1d(no_of_channels, output_channels, kernel_size, stride=1, padding=1)
            self.soft = nn.Softmax(dim=3)
    
        def parzen_window_estimate2(self,x,):
            """
            Perform Parzen window estimate with Gaussian kernel for multivariate PDF.

            Parameters:
            - data: Tensor of shape (frames, points, channels) NT , V, C 
            - h: Bandwidth parameter for the Gaussian kernel

            Returns:
            - pdf_estimates: Tensor of shape (frames, points)
            """
            N,V,C= x.shape
            # Expand h_values to match the shape of kernel_values
            
            h = self.h_values.unsqueeze(0).unsqueeze(0)

            # Calculate Gaussian kernel values for each data point
            kernel_values = torch.exp(-0.5 * ((x.unsqueeze(1) - x.unsqueeze(2)) / h) ** 2)/h.squeeze(1).unsqueeze(2)
            
            kernel_values = kernel_values * self.weights
            kernel_values = torch.prod(kernel_values, dim=-1) *self.joint_weights
            ...
            pdf_estimates = kernel_sum / (torch.sqrt((2 * torch.tensor(3.141592653589793)) ** C) )
        ...
        def forward (self,x):
            ...
            if len(x.shape) == 4:
                ...
                x = self.m(x.permute(0,2,1)).permute(0,2,1)
                density = self.parzen_window_estimate2(x).cuda()
                density = density.contiguous().view(N,T,V)
                ...
                x = self.m(x.permute(0,2,1)).permute(0,2,1)
                density = self.parzen_window_estimate2(x).cuda()
                density = density.contiguous().view(N*M,T,V)

            density_diff = density [:, 1:, :] - density [:, :-1, :]  # N T V 

            # # Set the first element along the frame dimension to 0
            density_diff = torch.cat([torch.zeros_like(density_diff [:, :1, :]), density_diff ], dim=1)
            
            
            density = abs(density_diff .unsqueeze(1))
...
class Transformer(nn.Module):
    def __init__(self, dim_in=3, dim_feat=256, decoder_dim_feat=256,
        ...
        self.gen_mask = Gen_mask(no_of_channels=12,output_channels=16, no_of_joints=25,kernel_size = 3)
        # --------------------------------------------------------------------------
    ...
    def motion_aware_random_masking(self, x, x_orig, mask_ratio, tau):
        ...
        _, TP, VP, _ = x_orig.shape  #64, 30, 25, 12 NM, TP, V, C* patch size in coordinate space 
        ...
        xx = self.gen_mask(x_orig).mean(dim=1) 
        x_orig_motion = xx

        # x_orig_motion = x_orig_motion.mean(dim=[3])  # NM, TP, VP uncomment this for original implementation.
    ...
    def forward_encoder(self, x, mask_ratio, motion_aware_tau):
        ...
        x_orig = self.patchify(x) # 64, 750, 12 NM, TP*VP,C
    ...
    def forward(self, x, mask_ratio=0.80, motion_stride=1, motion_aware_tau=0.75, **kwargs):
        ...
        x = x.permute(0, 4, 2, 3, 1).contiguous().view(N * M, T, V, C) # 64, 120, 25, 3
        # print('x',x.shape)

        x_motion = self.extract_motion(x, motion_stride) # 64, 120, 25, 3
        # with torch.no_grad():
        #     x_density=self.patchify(x).reshape(N*M,30,V,12)
        #     # print(x.shape)
        #     x_density = self.gen_mask(x_density)
            # jc
        # print('motion', x_motion.shape)
